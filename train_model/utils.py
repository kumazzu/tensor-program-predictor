...
def train_mlp(epoch, model, loss_func, data_loader, lr_scheduler, optimizer, device):
    ...
    for iter, (data, label) in enumerate(data_loader):
        ...
        predict.append(prediction.squeeze().cpu().detach().numpy().tolist())
        target.append(label.squeeze().cpu().detach().numpy().tolist())
        iter_tmp = iter
    lr_scheduler.step()
    epoch_loss /= iter_tmp + 1
    target = np.array([i for arr in target for i in arr])
    predict = np.array([i for arr in predict for i in arr])
...
def eval_mlp(epoch, model, loss_func, data_loader, device, isprint):
    ...
    with torch.no_grad():
        for iter, (data, label) in enumerate(data_loader):
            ...
            predict.append(prediction.squeeze().cpu().numpy().tolist())
            target.append(label.squeeze().cpu().numpy().tolist())
            iter_tmp = iter
        test_loss /= iter_tmp + 1
    target = np.array([i for arr in target for i in arr])
    predict = np.array([i for arr in predict for i in arr])
    if isprint:
